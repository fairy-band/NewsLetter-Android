...
사용자가 요청한 작업에 대한 계획서를 `.Codex/plans/` 폴더에 생성한다.
...
`AGENTS.md`를 읽어 아키텍처, 모듈 구조, 네이밍 컨벤션을 파악한다.
...
### 3. .Codex/plans/ 디렉토리 준비

- `.Codex/plans/` 폴더가 없으면 생성한다
- `.gitignore`에 `.Codex/plans/` 항목이 없으면 추가한다 (계획서는 Git에 포함되지 않음)

### 4. 계획서 생성

파일명: `.Codex/plans/[기능명-kebab-case].md`  
예: `.Codex/plans/home-refresh.md`, `.Codex/plans/fix-crash-on-launch.md`
